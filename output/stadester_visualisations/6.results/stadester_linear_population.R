...
# Minor X-axis ticks (Graticule lines)
minor_x_tick_years <- sort(
  unique(
    c(
      seq(-10000, 0, by = 1000),
      seq(0, 1700, by = 100),
...
final_plot <- ggplot(
  ...
  # Axes configuration with graticule breaks
  scale_x_continuous(
    breaks = x_tick_scaled,
    minor_breaks = minor_x_tick_scaled,
    labels = x_tick_years,
    expand = c(0.01, 0.01)
  ) +
  scale_y_continuous(
  ...
  scale_shape_manual(
    ...
    labels = c(
      ...
      "Rural Population" = "Rural & Periurban Population"
    )
  ) +
  # Legend formatting
  guides(
